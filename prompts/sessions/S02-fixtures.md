...
Сделай:
1. `Report` для тестового комплекта из `data/case11/*.txt` (это те же docx текстом): реальные названия из документов — «Блок внутреннего аудита», «Главный аудитор», подразделения из раздела 3, ДЗО; 3 UnitChange (сохранено, преобразовано, создано), 8 FunctionMatch (по два kept/changed/lost/new/moved), 2 Duplicate, 1 Conflict (правило «выполняет и контролирует»), `conclusion_md` на 10 строк, `stats`. Все `sources` — с настоящими номерами пунктов и цитатами из `data/case11/*.txt` (скопируй дословно, 1–2 предложения).
2. Тот же объект в `backend/app/mocks/demo_report.json`.
3. `frontend/lib/fixtures/run.ts` — RunStatus для трёх состояний: parsing 20%, matching 70%, done 100%.
