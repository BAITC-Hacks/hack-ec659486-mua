...
2. `components/run-progress.tsx`: пропсы `{ status: RunStatus }`; пять шагов «Разбор → Подразделения → Функции → Сопоставление → Заключение», соответствие статусов `parsing/units/functions/matching/conclusion`, `queued` — «в очереди»; пройденные, текущий (спиннер) и будущие шаги визуально различимы; полоса `progress` в процентах. `error` — красный блок с текстом `status.error` как есть (там уже русский текст бэкенда), ничего не дописывай.
