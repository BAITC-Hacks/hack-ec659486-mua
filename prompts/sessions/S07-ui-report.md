...
Сделай:
1. `page.tsx`: берёт `Report` из фикстуры, показывает `run_id`, документы «до»/«после» и `stats`; четыре вкладки: Подразделения, Функции, Дубли и конфликты, Заключение. Локальное состояние `openSource: Source | null` передаётся во все компоненты как `onSource(source)`.
...
4. `function-matrix.tsx`: таблица `FunctionMatch[]` — текст функции до, текст после, подразделение, статус, `confidence` в процентах, источники обоих пунктов; фильтр по статусу (Все / сохранена / изменена / утрачена / новая / перенесена) со счётчиком строк.
5. `duplicates-list.tsx` и `conflicts-list.tsx`: дубли — пара функций с подразделениями, `similarity`, `note`, источники; конфликты — `title`, `rule_id`, подразделения, `explanation`, функции с источниками. Находку без `sources` не рендерить.
