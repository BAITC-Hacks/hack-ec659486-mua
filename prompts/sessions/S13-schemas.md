...
В JSON-схемах вызовов отсутствующая строка — `""` (strict требует все поля); конвертеры в модели домена (`schemas.py`) превращают `""` в `None`. Это единственное допустимое расхождение между слоями.
